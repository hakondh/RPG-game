...
public class Weapon implements Equipable {
	...
	// Get the base damage, which will depend on both the type and the level of the
	// weapon
	public int getBaseDamage() {
		return weaponStrategy.getBaseDamage(level);
	}

	// Get the bonus damage, which will depend on both the type of weapon and the
	// stats of the Hero
	...
	public void displayStats() {
		...
		str.append("\n").append("Item stats for: ").append(name).append("\nWeapon type: ").append(weaponClass)
				.append("\nWeapon level: ").append(level).append("\nDamage: ").append(getBaseDamage());
